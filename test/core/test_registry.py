...
@pytest.mark.skipif(
    len(BAM2COV.available_methods) == 0 or len(SRA2FASTQ.available_methods) == 0,
...
def test_registry_with_dependencies():
    ...
    assert str(err.value) == "'an other converter already exists for BAM -> COV'"
...
def test_registry_with_less_dependencies():
    ...
    assert str(err.value) == "'an other converter already exists for FASTQ -> FASTA'"
