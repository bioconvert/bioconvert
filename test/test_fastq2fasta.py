...
def test_conv(method):
    ...
    expected_outfile = f"{test_dir}/data/fasta/test_fastq2fasta_v1.fasta"
    ...
    with TempFile(suffix=".fasta") as outfile, TempFile(suffix=".fasta") as unwrapped:
        convert = FASTQ2FASTA(infile, outfile.name)
        convert(method=method)
        FASTQ2FASTA.unwrap_fasta(outfile.name, unwrapped.name, strip_comment=True)
        assert md5(unwrapped.name) == md5out, "{} failed".format(method)


@pytest.mark.parametrize(
    "method", filter(make_in_gz_tester(FASTQ2FASTA), FASTQ2FASTA.available_methods)
)
@pytest.mark.skipif(
...
def test_in_gz(method):
    for sample_name in ["test_fastq2fasta_v1", "sample_v2", "sample_v3", "sample_v4"]:
        infile = f"{test_dir}/data/gz/{sample_name}.fastq.gz"

        expected_outfile = f"{test_dir}/data/fasta/{sample_name}.fasta"
        with TempFile(suffix=".fasta") as expected_unwrapped:
            FASTQ2FASTA.unwrap_fasta(
                expected_outfile, expected_unwrapped.name, strip_comment=True
            )
        ...
        # Some methods may output multi-line fasta, so we need to
        # compare md5 sums of unwrapped versions.
        with TempFile(suffix=".fasta") as outfile, TempFile(
            suffix=".fasta"
        ) as unwrapped:
            convert = FASTQ2FASTA(infile, outfile.name)
            convert(method=method)
            FASTQ2FASTA.unwrap_fasta(outfile.name, unwrapped.name, strip_comment=True)
            assert md5(unwrapped.name) == md5out, "{} failed for {}".format(
                method, sample_name
            )
...
def test_more_samples(method):
    for sample_name in ["sample_v2", "sample_v3", "sample_v4"]:
        ...
        infile = f"{test_dir}/data/fastq/{sample_name}.fastq"
